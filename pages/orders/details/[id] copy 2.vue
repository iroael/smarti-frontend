...
// Calculated values based on order data
    ...
    <div v-else-if="order" class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
              ...
              <div class="text-right">
                <p class="text-2xl font-bold text-gray-900">{{ formatCurrency(calculateGrandTotal()) }}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
                        ...
                        <div class="flex items-center mt-2 space-x-4">
                          ...
                          <span class="text-xs text-gray-500">Stock: {{ item.product.stock }}</span>
                        ...
                        <!-- Tax Information -->
                        <div v-if="item.taxes && item.taxes.length > 0" class="mt-2">
                          <div v-for="tax in item.taxes" :key="tax.id" class="text-xs text-gray-500">
                      ...
                      <div class="text-right ml-4">
                        ...
                        <p class="text-sm font-semibold text-gray-900 mt-1">{{ formatCurrency(calculateItemTotal(item)) }}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="mt-6 pt-6 border-t border-gray-200">
                <div class="space-y-2">
                  ...
                  <div class="flex justify-between text-sm">
                    ...
                    <span class="text-gray-900">{{ formatCurrency(order.shippingCost) }}</span>
                  </div>
                  <div class="flex justify-between text-base font-medium pt-2 border-t border-gray-200">
                    <span class="text-gray-900">Total</span>
                    <span class="text-gray-900">{{ formatCurrency(calculateGrandTotal()) }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="bg-white shadow rounded-lg">
            ...
            <div class="px-6 py-4 space-y-4">
              <div>
                ...
                <p class="text-sm text-gray-900">{{ order.supplier.supplier_code }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Name</label>
                <p class="text-sm text-gray-900">{{ order.supplier.name }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Category</label>
                <p class="text-sm text-gray-900">{{ order.supplier.kategori }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Email</label>
                <p class="text-sm text-gray-900">{{ order.supplier.email }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Phone</label>
                <p class="text-sm text-gray-900">{{ order.supplier.phone }}</p>
              </div>
              <div>
                <label class="text-sm font-medium text-gray-500">Address</label>
                <p class="text-sm text-gray-900">{{ order.supplier.address }}</p>
              </div>
            </div>
          </div>
          ...
          <!-- Order Notes -->
          <div v-if="order && order.notes" class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <div>
                          <span :class="event.completed ? 'bg-blue-500' : 'bg-gray-400'" class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white">
                            <svg class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                              <path v-if="event.completed" fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
