...
import { generateAvatar } from '../data/avatar'

export function getPartyColumn(partyType: 'customer' | 'supplier'): ColumnDef<Order> {
  return {
    id: partyType,
    header: ({ column }) =>
      h(DataTableColumnHeader, { column, title: partyType === 'customer' ? 'Customer' : 'Supplier' }),
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center gap-3' }, [
        avatarUrl
          ? h(Avatar, { class: 'h-8 w-8' }, [h(AvatarImage, { src: avatarUrl })])
          : h('div', {
